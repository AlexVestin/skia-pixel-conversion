...
//     float y = u_tex_size.y - coords.y;
//     float index = coords.x + (y - 0.5) * out_width - 0.5;
...
//     float v = dot(float4(color.rgb, 1.0), conv);
//     // return float4(floor(v * 255.) / 255.);
//     return float4(v);
// }
// )";
...
//             float2 position;
//             float4 conv;    
//             float2 step = float2(1.) / u_tex_size;
          
//             float4 color;
//             if (index < ustart) { 
//                 color = u_tex.eval(position);
//                 position = index_to_position(index, size, 1.0);
//                 conv = conversion_y;
//             } else { 
//                 position = index_to_position(index - (index < vstart ? ustart : vstart), size, 2.0);
...
int main()
{
    ...
    for(int i = 0; i < 2000; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            bool origError = std::abs(d[r] - yuv1[i * 4 + j]) > errorSize;
            bool gpuError = std::abs(d[r] - yuv2[i * 4 + j]) > errorSize;
            // if (gpuError) {
            //     std::cout << "Other values: " << +d[r + 0] << " " << +d[r + 1] << " " << +d[r + 2] << " " << +d[r + 3] << std::endl;
